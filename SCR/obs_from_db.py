...
def to_grid(df, table="hailw"):
    ...
    for idx, row in df.iterrows():
        ...
        if progress_counter == progress_update_rate:
            progress_counter = 0
            progress.progress_print(idx, maxidx, label="Gridding Lightning Obs")
    # plt.imshow(var)
    # plt.colorbar()
    # plt.show()
...
def get_hail_from_sybase(start_date, end_date):
    ...
    connection = pyodbc.connect(
        driver = 'FreeTDS',
        server = server_to_hostname[server]['name'],
        port = server_to_hostname[server]['port'],
        database = 'wien',
        user = user,
        password = pswd,
        tds_version="5.0")
    ...
    con.execute(request)
    tmp_ = con.fetchall()
    hail = pd.DataFrame.from_records(tmp_, columns=hail_columns)
    hail = add_datetime(hail)
    hail = conv_lat_lon(hail)
    hail = hail.drop(['Datum', 'stdmin', 'breite', 'laenge'], axis=1)
    return(hail)
...
def get_lightning_from_db(start_dat, start_tim, end_dat, end_tim, retry=False):
    ...
    print("request: "+request)
    connection = pyodbc.connect(
        driver = 'FreeTDS', server = server_to_hostname[server]['name'], port = server_to_hostname[server]['port'],
        database = 'wien', user = user, password = pswd) #, tds_version="5.0")
    con = connection.cursor()
    con.execute(request)
    tmp_ = con.fetchall()
    if retry:
        lightning = pd.DataFrame.from_records(tmp_)
        lightning = add_datetime(lightning, table='blitz2')
        lightning = conv_lat_lon(lightning)
        print(lightning)
    else:
        lightning = pd.DataFrame.from_records(tmp_, columns=lightning_columns)
        lightning = add_datetime(lightning, table='blitz2')
        lightning = conv_lat_lon(lightning)
        lightning = lightning.drop(['Datum', 'stdmin', 'breite', 'laenge'], axis=1)
    return lightning
...
def read_hail(data_list, start_date, end_date):
    hail = read_archived_file(start_date, end_date, "lightning") # returns None if no archive or corrupt archive is found
