        ...
        if 'ensemble' in sim.keys():
            ...
            if sim['ensemble']:
                if not sim['ensemble'] in ensembles.keys():
                    ensembles[sim['ensemble']] = 1
                else:
                    ensembles[sim['ensemble']] += 1
        else:
            logger.info(f"Model {sim['name']} has NO ensemble key")
    for key, num in ensembles.items():
        logger.info(f"Found ensemble {key} with {num} members in input data.")
    return ensembles

def ens_pFSS(data_list, ensembles, args):
    rain_ens = {}
    score_ens = {}
    levels = parameter_settings.get_fss_thresholds(args)
    windows=[10,20,30,40,60,80,100,120,140,160,180,200]
    ny, nx = data_list[0]["precip_data_resampled"].shape
    windows = prep_windows(windows, args.fss_calc_mode, nx, ny)
    for ens_name, ens_members in ensembles.items():
        first = True
        mem = 0
        score_ens[ens_name] = {}
        for sim in data_list:
            if 'ensemble' in sim.keys():
                if sim['ensemble'] == ens_name:
                    logger.info(f"Adding data from {sim['name']} to ensemble {ens_name} at index {mem}")
                    if first:
                        rain_ens[ens_name] = np.zeros((ens_members, *sim['precip_data_resampled'].shape))
                        first = False
                    rain_ens[ens_name][mem, :, :] = sim['precip_data_resampled']
                    mem += 1
        score_ens[ens_name]['pFSS_num'], score_ens[ens_name]['pFSS_den'], score_ens[ens_name]['pFSS_fss'], score_ens[ens_name]['pFSS_ovest'] = fss_functions.fss_frame_eps(
            rain_ens[ens_name], data_list[0]['precip_data_resampled'], windows, levels)
    return score_ens
